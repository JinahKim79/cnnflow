...
% Authors: Andrea Vedaldi, modified by Damien Teney
...
% Check for parameters defined multiple times
givenArgs = lower(args(1:2:end)) ;
uniqueGivenArgs = unique(givenArgs) ;
for i = 1:length(uniqueGivenArgs)
  for j = 1:length(givenArgs)
    if isequal(givenArgs(j), uniqueGivenArgs(i))
      givenArgs(j) = [] ; % Remove it
      break ; % Remove it ONCE only !
    end
  end
end
if ~isempty(givenArgs)
  error(sprintf('Argument(s) set multiple times:%s', sprintf('\n\t%s ', givenArgs{:}))) ;
end
