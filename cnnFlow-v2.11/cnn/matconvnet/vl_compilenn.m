...
root = fileparts(mfilename('fullpath')) ; % Teney's modification
addpath(fullfile(root, 'src')) ; % Teney's modification
...
lib_src{end+1} = fullfile(root,'src','bits',['data.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['datamex.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnconv.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnfullyconnected.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnsubsample.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnpooling.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnnormalize.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnbnorm.' ext]) ;
lib_src{end+1} = fullfile(root,'src','bits',['nnbias.' ext]) ;
mex_src{end+1} = fullfile(root,'src',['vl_nnconv.' ext]) ;
mex_src{end+1} = fullfile(root,'src',['vl_nnconvt.' ext]) ;
mex_src{end+1} = fullfile(root,'src',['vl_nnpool.' ext]) ;
mex_src{end+1} = fullfile(root,'src',['vl_nnnormalize.' ext]) ;
mex_src{end+1} = fullfile(root,'src',['vl_nnbnorm.' ext]) ;

% CPU-specific files
lib_src{end+1} = fullfile(root,'src','bits','impl','im2row_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','impl','subsample_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','impl','copy_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','impl','pooling_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','impl','normalize_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','impl','bnorm_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','impl','tinythread.cpp') ;
lib_src{end+1} = fullfile(root,'src','bits','imread.cpp') ;

% GPU-specific files
if opts.enableGpu
  lib_src{end+1} = fullfile(root,'src','bits','impl','im2row_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','subsample_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','copy_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','pooling_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','normalize_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','bnorm_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','datacu.cu') ;
end

% cuDNN-specific files
if opts.enableCudnn
  lib_src{end+1} = fullfile(root,'src','bits','impl','nnconv_cudnn.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','nnbias_cudnn.cu') ;
  lib_src{end+1} = fullfile(root,'src','bits','impl','nnpooling_cudnn.cu') ;
end

% Other files
if opts.enableImreadJpeg
  mex_src{end+1} = fullfile(root,'src', ['vl_imreadjpeg.' ext]) ;
  lib_src{end+1} = fullfile(root,'src', 'bits', 'impl', ['imread_' opts.imageLibrary '.cpp']) ;
end
...
% Build directories
mex_dir = fullfile(root, 'mex') ; % Teney's modification
bld_dir = fullfile(root, 'mex', '.build', filesep); % Teney's modification
...
%vl_setupnn() ; % Teney's modification
...
% --------------------------------------------------------------------
str = fullfile('src') ; % Teney's modification
...
switch arch
  case {'win64'}
    config_dir = fullfile(matlabroot, 'toolbox', ...
                          ...
                          'src', 'mex', arch) ;
  case {'maci64', 'glnxa64'}
    config_dir = fullfile(root, 'src', 'config') ; % Teney's modification
end
